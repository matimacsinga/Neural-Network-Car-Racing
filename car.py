...
class Car:
    ...
    def initImgs(self):
        img_names = ["orange_car.png", "pink_car.png", "blue_car.png", "green_car.png"]
...
def getSensorEquations(self, world):       
    ...
    for i in range(4):
        ...
        if CAR_DBG:             
            py.draw.lines(world.win, RED, False, [world.getScreenCoords(self.x+dx, self.y+dy), world.getScreenCoords(self.x-dx, self.y-dy)], 2)

        coef = getSegmentEquation(self, Vector(x = self.x+dx, y = self.y+dy))
        eq.append(coef)
    return eq
