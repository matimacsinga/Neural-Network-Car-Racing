...
class Track:
    def __init__(self, world):
        ...
        for i in range(self.num_ctrl_points):
             self.ctrl_points.append(Vector())

        for i in range(NUM_POINTS*self.num_ctrl_points):        
            self.pointsLeft.append(Vector(1000,1000))
            self.pointsRight.append(Vector(1000,1000))
            self.centerPoints.append(Vector(1000,1000))
    ...
    def draw(self, world):
        if(ROAD_DBG):
            for i in range(len(self.pointsLeft)):
                py.draw.circle(world.win, MIDDLE_NODES_COLOR, world.getScreenCoords(self.pointsLeft[i].x, self.pointsLeft[i].y), 2)
                py.draw.circle(world.win, MIDDLE_NODES_COLOR, world.getScreenCoords(self.pointsRight[i].x, self.pointsRight[i].y), 2)
        else:
            for i in range(len(self.pointsLeft)):
